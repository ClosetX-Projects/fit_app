...
export function ProfessorView() {
  ...
  // A API já filtra automaticamente pelo token do professor logado
  const { data: students, loading: isLoading } = useApi<any[]>(user ? `/users/alunos` : null);
    ...
    <>
      ...
      <nav className="fixed bottom-0 left-0 right-0 z-50 bg-background/90 backdrop-blur-lg border-t flex justify-around items-center h-[calc(4rem+env(safe-area-inset-bottom))] px-1 pb-safe md:hidden">
        {[
          ...
          { id: 'library', label: 'Programas', icon: BookOpen },
          { id: 'avaliacao', label: 'Avaliação', icon: ClipboardList }
        ].map((item) => {
          const Icon = item.icon;
          let isActive = false;
          
          if (item.id === 'avaliacao') {
             isActive = showAssessmentWizard;
          } else {
             isActive = !showAssessmentWizard && !selectedStudentId && activeTab === item.id;
          }
